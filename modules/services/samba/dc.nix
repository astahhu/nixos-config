...
in
{
  options.astahhu.services.samba.dc = {
    ...
    dhcp = {
      enable = lib.mkEnableOption "Enable Kea DHCP";

      dns-servers = lib.mkOption { type = lib.types.listOf lib.types.str; };
      time-servers = lib.mkOption { type = lib.types.listOf lib.types.str; };
      routers = lib.mkOption { type = lib.types.listOf lib.types.str; };
  ...
  config = lib.mkIf dc.enable {
    ...
    // (lib.mapAttrs' (name: _: {
      ...
      value = {
        backup = true;
        path = "${config.nix-tun.storage.persist.path}/samba-shares/${name}-dfs";
        mode = "0555";
      };
    }) dc.domain-dfs);
    ...
    services.samba = {
      ...
      settings = {
        ...
        global = {
          ...
          "server services" = "s3fs, rpc, ldap, cldap, kdc, drepl, ntp_signd, kcc, dnsupdate";
